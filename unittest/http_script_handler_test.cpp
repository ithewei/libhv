...
static std::string write_script(const char* name, const char* content) {
    std::string dir = "tmp/http_script_handler_test";
    const char* slash = strrchr(name, '/');
    if (slash) {
        dir = HPath::join(dir, std::string(name, slash - name));
    }
    hv_mkdir_p(dir.c_str());
    std::string path = HPath::join("tmp/http_script_handler_test", name);
    HFile file;
...
static void test_script_dir_mapping() {
    ...
    service.Script("/api/", "tmp/http_script_handler_test/api");
...
int main() {
    ...
    printf("ALL http_script_handler_test PASSED\n");
    return 0;
}
