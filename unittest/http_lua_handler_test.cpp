/*
 * http_lua_handler_test — Stage B integration test for the coroutine-based
 * HttpLuaHandler: proves .lua HTTP handlers run as coroutines concurrently on
 * a single IO thread.
 *
...
static std::string write_script(const char* name, const char* content) {
    hv_mkdir_p("tmp/http_lua_handler_test");
