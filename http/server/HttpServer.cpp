...
static void http_server_start_before_run(void* userdata) {
...
void http_server_run_with_start(http_server_t* server) {
    HttpServerPrivdata* privdata = (HttpServerPrivdata*)server->privdata;

    auto loop = privdata->loops[0];
...
int http_server_run(http_server_t* server, int wait) {
    ...
    if (server->worker_processes) {
        ...
        if (wait) {
            http_server_start_before_run(server);
        }
        return 0;
    }
}
